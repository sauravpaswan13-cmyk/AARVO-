...
@Composable private fun RiderScreen(api:AarvoApiClient,onBack:()->Unit){var jobs by remember{mutableStateOf<List<JSONObject>>(emptyList())};var notes by remember{mutableStateOf<List<JSONObject>>(emptyList())};var msg by remember{mutableStateOf("")};val scope=rememberCoroutineScope();fun reload(){scope.launch{try{jobs=api.riderAssignments().toList();notes=api.riderNotifications().toList()}catch(t:Throwable){msg=t.message?:"Unable to load rider jobs"}}};LaunchedEffect(Unit){reload()};LazyColumn(Modifier.fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){item{Text("AARVO Rider",style=MaterialTheme.typography.headlineSmall,fontWeight=FontWeight.Bold);Text("Assigned delivery jobs")};item{Text("Notifications",style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.Bold)};items(notes){n->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp)){Text(n.optString("title"),fontWeight=FontWeight.Bold);Text(n.optString("body"))}}};item{Text("My jobs",style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.Bold)};items(jobs){a->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp)){Text("Order #"+a.optString("order_id"),fontWeight=FontWeight.Bold);Text("Status: "+a.optString("status"));Text("Buyer: "+a.optString("buyer_name")+" • "+a.optString("buyer_phone"));Text("Address: "+a.optString("address_line"));val next=when(a.optString("status")){"ASSIGNED"->"ACCEPTED";"ACCEPTED"->"PICKED_UP";"PICKED_UP"->"OUT_FOR_DELIVERY";"OUT_FOR_DELIVERY"->"DELIVERED";else->""};if(next.isNotBlank())Button(onClick={scope.launch{try{api.riderUpdateAssignment(a.optString("id"),next);reload()}catch(t:Throwable){msg="Delivery update failed"}}}){Text("Mark "+next)}}}};item{if(msg.isNotBlank())Text(msg,color=MaterialTheme.colorScheme.error)};item{Button(onClick=onBack,modifier=Modifier.fillMaxWidth()){Text("Close")}}}}
