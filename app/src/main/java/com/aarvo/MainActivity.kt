...
private fun AarvoRoot(context: Context) {
    ...
    when {
        !onboarded -> OnboardingScreen(onDone = { prefs.edit().putBoolean("onboarded", true).apply(); onboarded = true })
        !signedIn -> SignInScreen(onSignedIn = { name -> userName = name; prefs.edit().putBoolean("signed_in", true).putString("user_name", name).apply(); signedIn = true })
        else -> AarvoApp(userName = userName, onSignOut = { prefs.edit().putBoolean("signed_in", false).apply(); signedIn = false })
    }
}
...
private fun OnboardingScreen(onDone: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        ...
        Spacer(Modifier.height(8.dp)); Text("Discover useful products, save favourites and checkout in a simple shopping experience.")
...
private fun SignInScreen(onSignedIn: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        Text("Welcome to AARVO", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold); Spacer(Modifier.height(16.dp))
        OutlinedTextField(name, { name = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("Your name") }); Spacer(Modifier.height(10.dp))
        OutlinedTextField(email, { email = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("Email") }); Spacer(Modifier.height(16.dp))
        ...
        Spacer(Modifier.height(8.dp)); Text("Demo authentication: account data is stored locally on this device.", style = MaterialTheme.typography.bodySmall)
    }
}
...
private fun AarvoApp(userName: String, onSignOut: () -> Unit, cartViewModel: CartViewModel = viewModel()) {
    ...
    var orderPlaced by remember { mutableStateOf(false) }; var address by remember { mutableStateOf("") }; val cartItems by cartViewModel.items.collectAsState()
    val repository = remember { ProductRepository() }; val products = remember(query, category) { repository.products(query, category) }
    ...
    if (showCheckout) CheckoutDialog(cartItems.sumOf { it.price }, address, { address = it }, { showCheckout = false }, { showCheckout = false; orderPlaced = true; cartViewModel.clear() })
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("AARVO", fontWeight = FontWeight.Bold) }, actions = { BadgedBox(badge = { if (cartItems.isNotEmpty()) Badge { Text(cartItems.size.toString()) } }) { IconButton(onClick = { selectedTab = 1 }) { Icon(Icons.Default.ShoppingCart, "Cart") } } }) }, bottomBar = { NavigationBar {
...
private fun HomeScreen(padding: PaddingValues, query: String, onQueryChange: (String) -> Unit, categories: List<String>, selectedCategory: String, onCategoryChange: (String) -> Unit, products: List<Product>, onAdd: (Product) -> Unit, onOpen: (Product) -> Unit, wishlist: Set<Int>, onToggleWishlist: (Int) -> Unit) {
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item { Text("Shop smart. Live better.", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold); Text("Discover products you'll love.") }
        ...
        if (products.isEmpty()) item { Text("No products found. Try another search.") } else items(products, key = { it.id }) { product -> ProductCard(product, product.id in wishlist, onAdd, onOpen, onToggleWishlist) }
    }
}

@Composable
private fun ProductCard(product: Product, isSaved: Boolean, onAdd: (Product) -> Unit, onOpen: (Product) -> Unit, onToggleWishlist: (Int) -> Unit) {
    Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
        Text("${product.emoji}  ${product.name}", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold, modifier = Modifier.weight(1f)); IconButton(onClick = { onToggleWishlist(product.id) }) { Icon(if (isSaved) Icons.Default.Favorite else Icons.Default.FavoriteBorder, "Wishlist") }
...
private fun CartScreen(padding: PaddingValues, items: List<Product>, onRemove: (Product) -> Unit, onClear: () -> Unit, onCheckout: () -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        if (items.isEmpty()) item { Text("Your cart is empty. Add something you like from Home.") } else {
...
private fun CheckoutDialog(total: Int, address: String, onAddressChange: (String) -> Unit, onDismiss: () -> Unit, onPlaceOrder: () -> Unit) {
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Checkout") }, text = { Column(verticalArrangement = Arrangement.spacedBy(10.dp)) { Text("Order total: ₹$total", fontWeight = FontWeight.Bold); OutlinedTextField(address, onAddressChange, label = { Text("Delivery address") }, minLines = 3); Text("Payment: Cash on delivery (demo)", style = MaterialTheme.typography.bodySmall) } }, confirmButton = { Button(onClick = onPlaceOrder, enabled = address.isNotBlank()) { Text("Place order") } }, dismissButton = { TextButton(onClick = onDismiss) { Text("Cancel") } })
}

@Composable
private fun ProfileScreen(padding: PaddingValues, userName: String, address: String, onSignOut: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text("My Profile", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold); Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp)) { Text(userName, style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold); Text("AARVO account") } }
        Text("Orders", style = MaterialTheme.typography.titleMedium); Text("Order history will appear here after your purchases."); Text("Saved address", style = MaterialTheme.typography.titleMedium); Text(if (address.isBlank()) "No address saved yet." else address)
        Text("Seller/Admin tools and backend sync are planned for the next phase.", style = MaterialTheme.typography.bodySmall); TextButton(onClick = onSignOut) { Text("Sign out") }
    }
}
