...
private fun AarvoRoot(activity: MainActivity, context: Context) {
    ...
    when {
        ...
        else -> AarvoApp(userName, api, activity, onSignOut = {
...
private fun AarvoApp(userName: String, api: AarvoApiClient, activity: MainActivity, onSignOut: () -> Unit, cartViewModel: CartViewModel = viewModel()) {
    ...
    if (showCheckout) CheckoutDialog(
        ...
        onDismiss = { if (!checkoutLoading) { showCheckout = false; checkoutMessage = "" } },
        onPlaceOrder = { fullName, phone, line1, city, state, postalCode ->
            checkoutLoading = true
            checkoutMessage = "Creating secure order..."
            scope.launch {
                try {
                    ...
                    val address = JSONObject().apply {
                        put("fullName", fullName.trim()); put("phone", phone.trim()); put("line1", line1.trim())
                        put("line2", ""); put("city", city.trim()); put("state", state.trim()); put("postalCode", postalCode.trim()); put("country", "IN")
                    }
                    val order = api.createOrder(items, address)
                    val options = JSONObject().apply {
                        put("key", order.getString("keyId")); put("amount", order.getLong("amountPaise")); put("currency", order.getString("currency"))
                        put("name", "AARVO"); put("description", "AARVO marketplace order"); put("order_id", order.getString("gatewayOrderId"))
                        put("prefill", JSONObject().put("name", fullName.trim()).put("contact", phone.trim()))
                        put("notes", JSONObject().put("order_id", order.getString("orderId")))
                    }
                    ...
                    activity.startRazorpayPayment(options) { paymentId, paymentError ->
                        scope.launch {
                            if (paymentId != null) {
                                try {
                                    api.verifyPayment(order.getString("orderId"), paymentId, order.getString("gatewayOrderId"), options.optString("razorpay_signature", ""))
                                } catch (_: Throwable) { }
                            }
                        }
                    }
                    ...
                    checkoutLoading = false
                    checkoutMessage = t.message ?: "Unable to create order."
                }
            }
        }
    )
...
private fun CartScreen(padding: PaddingValues, items: List<Product>, onRemove: (Product) -> Unit, onClear: () -> Unit, onCheckout: () -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        if (items.isEmpty()) item { Text("Your cart is empty. Add something you like from Home.") }
            ...
            item { Text("Total: ₹$total", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold); Button(onClick = onCheckout) { Text("Proceed to secure checkout") } }
        }
    }
}
...
private fun CheckoutDialog(total: Int, loading: Boolean, message: String, onDismiss: () -> Unit, onPlaceOrder: (String, String, String, String, String, String) -> Unit) {
    ...
    AlertDialog(
        ...
        text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text("Cart value: ₹$total", fontWeight = FontWeight.Bold)
            ...
            Text("Payment is processed by Razorpay. AARVO verifies the payment on its server before confirming the order.", style = MaterialTheme.typography.bodySmall)
        } },
        confirmButton = { Button(onClick = { onPlaceOrder(fullName, phone, line1, city, state, postalCode) }, enabled = !loading && fullName.isNotBlank() && phone.trim().length >= 10 && line1.isNotBlank() && city.isNotBlank() && state.isNotBlank() && postalCode.trim().length >= 5) { if (loading) CircularProgressIndicator() else Text("Pay securely") } },
        dismissButton = { TextButton(onClick = onDismiss, enabled = !loading) { Text("Close") } }
    )
}

@Composable
private fun ProfileScreen(padding: PaddingValues, userName: String, onSignOut: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Text("Orders", style = MaterialTheme.typography.titleMedium); Text("Your paid orders are recorded on the AARVO server.")
