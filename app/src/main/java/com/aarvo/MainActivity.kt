...
class MainActivity : ComponentActivity(), PaymentResultWithDataListener {
    ...
    fun startRazorpayPayment(options: JSONObject, callback: (paymentId: String?, error: String?) -> Unit) {
        PaymentBridge.clear()
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        ...
        Button(onClick = {
            loading = true; error = ""; scope.launch { try {
                ...
                val user = result.getJSONObject("user"); onSignedIn(user.optString("display_name", name), result.getString("token"))
...
@Composable
private fun AarvoApp(userName: String, api: AarvoApiClient, activity: MainActivity, onSignOut: () -> Unit, cartViewModel: CartViewModel = viewModel()) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("AARVO", fontWeight = FontWeight.Bold) }, actions = { BadgedBox(badge = { if (cartItems.isNotEmpty()) Badge { Text(cartItems.size.toString()) } }) { IconButton(onClick = { selectedTab = 1 }) { Icon(Icons.Default.ShoppingCart, "Cart") } } }) }, bottomBar = { NavigationBar {
        ...
        else -> ProfileScreen(padding, userName, onSignOut)
    } }
}

private fun JSONArray.toProductList(): List<Product> = buildList { for (i in 0 until length()) { val o = getJSONObject(i); add(Product(o.getLong("id").toInt(), o.getString("seller_id"), o.getString("seller_name"), o.getString("name"), o.getString("category"), o.getInt("price_paise") / 100, o.optDouble("rating", 0.0), "🛍️", o.getString("description"), o.getInt("stock_quantity"), o.optBoolean("is_published", true))) } }
...
private fun CheckoutDialog(total: Int, loading: Boolean, message: String, onDismiss: () -> Unit, onPlaceOrder: (String, String, String, String, String, String) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Secure checkout") }, text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) { Text("Cart value: ₹$total", fontWeight = FontWeight.Bold); OutlinedTextField(fullName, { fullName = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("Full name") }); OutlinedTextField(phone, { phone = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("Phone") }); OutlinedTextField(line1, { line1 = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("Address") }); OutlinedTextField(city, { city = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("City") }); OutlinedTextField(state, { state = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("State") }); OutlinedTextField(postalCode, { postalCode = it }, Modifier.fillMaxWidth(), singleLine = true, label = { Text("PIN code") }); if (message.isNotBlank()) Text(message, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.primary); Text("Payment is processed by Razorpay. AARVO verifies it on the server before confirming the order.", style = MaterialTheme.typography.bodySmall) } }, confirmButton = { Button(onClick = { onPlaceOrder(fullName, phone, line1, city, state, postalCode) }, enabled = !loading && fullName.isNotBlank() && phone.trim().length >= 10 && line1.isNotBlank() && city.isNotBlank() && state.isNotBlank() && postalCode.trim().length >= 5) { if (loading) CircularProgressIndicator() else Text("Pay securely") } }, dismissButton = { TextButton(onClick = onDismiss, enabled = !loading) { Text("Close") } })
}

@Composable
private fun ProfileScreen(padding: PaddingValues, userName: String, onSignOut: () -> Unit) { Column(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { Text("My Profile", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold); Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp)) { Text(userName, style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold); Text("AARVO account") } }; Text("Orders", style = MaterialTheme.typography.titleMedium); Text("Your paid orders are recorded on the AARVO server."); Text("Seller publishing requires server-side verification and payout readiness.", style = MaterialTheme.typography.bodySmall); TextButton(onClick = onSignOut) { Text("Sign out") } } }
