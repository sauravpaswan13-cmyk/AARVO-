...
class AarvoApiClient(
    ...
    private val client = OkHttpClient.Builder().connectTimeout(20, TimeUnit.SECONDS).readTimeout(30, TimeUnit.SECONDS).writeTimeout(30, TimeUnit.SECONDS).retryOnConnectionFailure(true).build()
    private val jsonMediaType = "application/json; charset=utf-8".toMediaType()
    fun isConfigured(): Boolean = runCatching { val uri = URI(baseUrl); uri.scheme.equals("https", ignoreCase = true) && !uri.host.isNullOrBlank() && uri.query == null && uri.fragment == null }.getOrDefault(false)
    private fun buildUrl(path: String): String { require(isConfigured()) { "AARVO live API is not configured in this build. Set aarvoApiBaseUrl to an HTTPS API URL." }; require(path.startsWith('/')) { "API path must start with /" }; require(!path.contains("..")) { "API path contains an invalid segment" }; return baseUrl + path }
    suspend fun health(): JSONObject = getObject("/health")
    suspend fun compareProducts(productIds: List<Int>): JSONObject { require(productIds.size in 2..3) { "Select 2 or 3 products to compare" }; require(productIds.all { it > 0 }) { "Product IDs must be positive" }; return post("/v1/ai/compare", JSONObject().put("productIds", JSONArray(productIds))) }\n    suspend fun aiAssistant(message: String): JSONObject { require(message.trim().isNotBlank()) { "Ask AARVO AI a question" }; return post("/v1/ai/assistant", JSONObject().put("message", message.trim())) }
    ...
    suspend fun verifyMsg91AccessToken(phone: String, accessToken: String): JSONObject { val normalizedPhone = IndianPhoneValidator.isValidOrThrow(phone); require(accessToken.trim().isNotBlank()) { "MSG91 access token is required" }; return post("/v1/auth/verify-msg91-token", JSONObject().put("phone", normalizedPhone).put("accessToken", accessToken.trim())) }
    ...
    suspend fun markProductViewed(productId: Int): JSONObject { require(productId > 0) { "Product ID must be positive" }; return post("/v1/products/$productId/viewed", JSONObject()) }
    suspend fun recentlyViewed(): JSONArray = get("/v1/products/recently-viewed")
    suspend fun requestReturn(orderId: String, reason: String, details: String = ""): JSONObject { require(orderId.trim().isNotBlank()) { "Order ID is required" }; require(reason.trim().isNotBlank()) { "Return reason is required" }; return post("/v1/orders/${orderId.trim()}/return-request", JSONObject().put("reason",reason.trim()).put("details",details.trim())) }
    suspend fun invoice(orderId: String): JSONObject { require(orderId.trim().isNotBlank()) { "Order ID is required" }; return getObject("/v1/orders/${orderId.trim()}/invoice") }
    suspend fun notifications(): JSONArray = get("/v1/notifications")
    suspend fun markNotificationRead(id: Long): JSONObject { require(id > 0) { "Notification ID is required" }; return post("/v1/notifications/$id/read", JSONObject()) }
    suspend fun createSupportTicket(subject: String, details: String, orderId: String? = null): JSONObject { require(subject.trim().isNotBlank()) { "Support subject is required" }; require(details.trim().length >= 5) { "Support details are required" }; return post("/v1/support/tickets", JSONObject().put("subject",subject.trim()).put("details",details.trim()).apply { if (!orderId.isNullOrBlank()) put("orderId",orderId.trim()) }) }
    ...
    suspend fun cancelOrder(orderId: String, reason: String = "ORDERED_BY_MISTAKE", details: String = ""): JSONObject { require(orderId.trim().isNotBlank()) { "Order ID is required" }; require(reason.trim().isNotBlank()) { "Cancellation reason is required" }; return post("/v1/orders/${orderId.trim()}/cancel", JSONObject().put("reason", reason.trim()).put("details", details.trim())) }
    suspend fun returnOrder(orderId: String, reason: String, details: String = ""): JSONObject { require(orderId.trim().isNotBlank()) { "Order ID is required" }; require(reason.trim().isNotBlank()) { "Return reason is required" }; return post("/v1/orders/${orderId.trim()}/return", JSONObject().put("reason", reason.trim()).put("details", details.trim())) }
    ...
    suspend fun sellerOnboarding(): JSONObject = getObject("/v1/seller/onboarding")
    suspend fun saveSellerOnboarding(payload: JSONObject): JSONObject = put("/v1/seller/onboarding", payload)
    ...
    suspend fun updateSellerProduct(productId: Int, name: String, category: String, pricePaise: Long, description: String): JSONObject { require(productId > 0) { "Product ID must be positive" }; require(name.trim().isNotBlank()) { "Product name is required" }; require(category.trim().isNotBlank()) { "Product category is required" }; require(pricePaise > 0) { "Product price must be positive" }; require(description.trim().isNotBlank()) { "Product description is required" }; return put("/v1/seller/products/$productId", JSONObject().put("name", name.trim()).put("category", category.trim()).put("pricePaise", pricePaise).put("description", description.trim())) }
    suspend fun deleteSellerProduct(productId: Int): JSONObject { require(productId > 0) { "Product ID is required" }; return delete("/v1/seller/products/$productId") }
    suspend fun publishSellerProduct(productId: Int): JSONObject { require(productId > 0) { "Product ID is required" }; return post("/v1/seller/products/$productId/publish", JSONObject()) }
    suspend fun updateInventory(productId: Int, stockQuantity: Int): JSONObject { require(productId > 0) { "Product ID is required" }; require(stockQuantity >= 0) { "Product stock cannot be negative" }; return post("/v1/seller/products/$productId/inventory", JSONObject().put("stockQuantity", stockQuantity)) }
    suspend fun productImageAdd(productId: Int, imageUrl: String, altText: String = "", sortOrder: Int = 0, isPrimary: Boolean = true): JSONObject { require(productId > 0) { "Product ID is required" }; require(imageUrl.startsWith("https://") || imageUrl.startsWith("http://")) { "Image URL must use HTTP or HTTPS" }; return post("/v1/seller/products/$productId/images", JSONObject().put("imageUrl", imageUrl.trim()).put("altText", altText.trim()).put("sortOrder", sortOrder).put("isPrimary", isPrimary)) }
    ...
    private suspend fun patch(path: String, payload: JSONObject): JSONObject = withContext(Dispatchers.IO) { JSONObject(execute(Request.Builder().url(buildUrl(path)).applyAuth().patch(payload.toString().toRequestBody(jsonMediaType)).build())) }
