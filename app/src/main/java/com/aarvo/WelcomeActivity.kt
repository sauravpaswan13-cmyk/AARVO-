...
private class ShoppingBagsView(context: Context) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        val w=width.toFloat(); val h=height.toFloat()
        fun bag(cx:Float,top:Float,bw:Float,bh:Float,c:Int){ fill.color=c; val body=Path().apply{moveTo(cx-bw/2,top+bh*.22f);lineTo(cx+bw/2,top+bh*.22f);lineTo(cx+bw*.42f,top+bh);lineTo(cx-bw*.42f,top+bh);close()};canvas.drawPath(body,fill);stroke.color=Color.rgb(82,42,130);val handle=Path().apply{moveTo(cx-bw*.25f,top+bh*.27f);cubicTo(cx-bw*.25f,top-bh*.08f,cx+bw*.25f,top-bh*.08f,cx+bw*.25f,top+bh*.27f)};canvas.drawPath(handle,stroke)}
        bag(w*.27f,h*.18f,w*.25f,h*.58f,Color.rgb(255,170,28)); bag(w*.50f,h*.08f,w*.29f,h*.72f,Color.rgb(242,62,119)); bag(w*.73f,h*.22f,w*.24f,h*.54f,Color.rgb(86,49,196))
    }
}
...
class WelcomeActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply {
            text = "AARVO"; textSize = 30f; setTextColor(Color.rgb(42,40,125)); gravity = Gravity.CENTER; setTypeface(typeface, Typeface.BOLD); letterSpacing = .03f
